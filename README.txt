HI
im new to github

Thank you for checking out the mod!

To be kept updated with the code, download CUSTOMBATTLER.lua (RECOMMENDED, do not have to redownload if CUSTOMBATTLER.lua is updated)
To view the code behind the project to edit yourself, download raw.lua (NOT RECOMMENDED, will have to redownload or reedit script if CUSTOMBATTLER.lua is updated)

Should work with any Roblox injector
Made by M4zterr
